...
const Carousel = () => {
    ...
    return (
        <div>
            <Slider {...settings}>
                {
                    data?.slice(90,107)?.map((item, index) => {
                        return <div key={index} className='bg-gradient-to-r from-[#0f0c29] via-[#302b63] to-[#24243e] -z-10'>
                            <div className='flex flex-col md:flex-row gap-10 justify-center h-[720px] my-20 md:my-0 items-center px-4'>
                                ...
                                <div>
                                    <img src={item.thumbnail} alt={item.title} className='bg-white rounded-full md:w-[550px] h-[550px]  w-[500px] hover:scale-105 transition-all shadow-2xl shadow-red-400'/>
                                </div>
                            </div>
                        </div>
                    })
